...
class Pairwise_ViT(nn.Module):
    ...
    def forward(self,text1, text2, x1, x2):
        ...
        feature2 = torch.concat([x2[:, 0, :],text2.squeeze(dim=1)],dim=-1)
        # Use the embedding of [CLS] token
        output2 = self.score_layer(feature2)
...
def train(args):
    ...
    for epoch in range(max_epoch):
        ...
        for ids, text1, text2, train_image1, train_image2, train_label in tqdm(data_loader):
            ...
            output,score1,score2 = model(text1, text2,train_image1, train_image2)
            # print(output)
    ...
    for id,text1, text2, image1,image2, label in tqdm(data_loader_test):
        output,score1,score2 = model(text1, text2,image1,image2)
